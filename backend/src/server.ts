...
app.patch('/api/chamados/:id', auth, requirePermission('calls.edit'), async (request: AuthRequest, response) => {
  const parsed = z.object({ orderNumber: z.string().trim().min(1).max(100).optional(), bdesk: z.string().trim().max(100).optional(), officeTrack: z.string().trim().max(100).optional(), client: z.string().trim().max(180).optional(), type: z.string().trim().max(100).optional(), reason: z.string().trim().max(5000).optional(), region: z.string().trim().max(100).optional(), city: z.string().trim().max(100).optional(), olt: z.string().trim().max(120).optional(), slotPon: z.string().trim().max(10000).optional(), status: z.enum(['Aberto', 'Atribuido', 'Deslocamento', 'Em campo', 'Finalizado']).optional(), technicianId: z.string().nullable().optional(), executedAt: z.string().datetime().optional(), result: z.string().trim().max(255).optional(), notes: z.string().max(5000).optional() }).safeParse(request.body);
