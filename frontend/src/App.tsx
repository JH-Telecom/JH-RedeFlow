          ...
          <div className="panel-heading">
            ...
            <button className="select-button">
              <div className="page-actions">
                <button className={`secondary-button compact ${syncing ? "is-refreshing" : ""}`} onClick={() => void syncDrive()} disabled={syncing}>
                  <Activity size={15}/> {syncing ? "Sincronizando..." : "Sincronizar Drive"}
                </button>
                <label className="primary-button compact file-button">
            </button>
          </div>
          <div className="chart">
...
function ImportsPage() {
  ...
  async function syncDrive() {
    ...
    try {
      const { sync } = await api.syncGoogleDrive();
      setMessage(`Sincronização concluída: ${sync.updated} chamados atualizados em ${sync.files} arquivo(s). ${sync.skipped} linha(s) ignorada(s).`);
      if (sync.errors.length) setError(sync.errors.join(" "));
    } catch (err) { setError(err instanceof Error ? err.message : "Nao foi possivel sincronizar o Google Drive."); }
